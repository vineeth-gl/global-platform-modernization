...
const { validateSku, ValidateSKU } = require("./validate"); // duplicated validators
...
app.post("/api/stock/reserve", async (req, res) => {
  ...
  // notify monolith — tight coupling
  try {
    await axios.post(`${MONOLITH}/api/v1/internal/inventory-hook`, {
      sku,
      qty,
      event: "reserve",
      ok: result.ok,
    }, { timeout: 2000, headers: { "X-Legacy-Bypass": "1" } });
  } catch (e) {
...
// duplicate of /api/stock/check with different route naming (inconsistent standards)
app.get("/Stock/GetAvailability", (req, res) => {
